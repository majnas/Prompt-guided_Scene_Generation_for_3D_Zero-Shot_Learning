...
class PointConvfeat(nn.Module):
    ...
    def forward(self, xyz, feat):
        ...
        fea = l3_points.view(B, 1024)
        return fea, _, _


class PointConvCls(nn.Module):
    def __init__(self, k=40):
        super(PointConvCls, self).__init__()
        feature_dim = 3
        self.sa1 = PointConvDensitySetAbstraction(npoint=512, nsample=32, in_channel=feature_dim + 3, mlp=[64, 64, 128], bandwidth = 0.1, group_all=False)
        self.sa2 = PointConvDensitySetAbstraction(npoint=128, nsample=64, in_channel=128 + 3, mlp=[128, 128, 256], bandwidth = 0.2, group_all=False)
        self.sa3 = PointConvDensitySetAbstraction(npoint=1, nsample=None, in_channel=256 + 3, mlp=[256, 512, 1024], bandwidth = 0.4, group_all=True)
...
class PointConvModel():
    def __init__(self, 
                k: int = 40, 
                logs_dir: str = "logs/PointConv",
                checkpoints_dir: str = 'checkpoints/PointConv',
                device: str = "cpu") -> None:

        self.classifier = PointConvCls(k=k)
    ...
    def train_one_epoch(self, train_dl):
        ...
        for i, (batch_points, batch_lables) in enumerate(pbar):
            ...
            jittered_data = provider.random_scale_point_cloud(batch_points[:,:, 0:3], scale_low=2.0/3, scale_high=3/2.0)
            jittered_data = provider.shift_point_cloud(jittered_data, shift_range=0.2)
            batch_points[:, :, 0:3] = jittered_data
            batch_points = provider.random_point_dropout_v2(batch_points)
            provider.shuffle_points(batch_points)
            ...
            pred = classifier(batch_points[:,:3,:],batch_points[:,3:,:])
    ...
    def save(self, epoch : int, save_best: bool = True) -> None:
        torch.save(self.classifier.state_dict(), f"{self.checkpoints_dir}/pointconv_cls_model_{epoch}_{self.test_acc_best}.pth")
        if save_best:
            # remove all previous best models
            [os.remove(f) for f in glob.glob(os.path.join(self.checkpoints_dir, "*best*"))]
            torch.save(self.classifier.state_dict(), f"{self.checkpoints_dir}/pointconv_cls_model_best_{self.test_acc_best}.pth")
